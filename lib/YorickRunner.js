...
module.exports.YorickRunner = (runnerConfig) => {
  const containerLsOutput = cp.execSync("docker container ls").toString();
  console.log({ containerLsOutput });
  ...
  async function run(suite = null) {
    ...
    try {
      const { name, dockerfile, runCommand, fileParamMatch, fileParamIgnore } =
        suite;
      ...
      const allCommands = fileParamMatches.map((match) =>
        render(runCommand, { file: match })
      );
      ...
      await Promise.all(
        chunks.map(async (chunkCommands, idx) => {
          const containerName = getContainerName(name, idx);

          try {
            cp.execSync(`docker container rm -f ${containerName}`);
          } catch (error) {
            // do nothing?
          }

          console.log({ containerName });
          await dockerUtils.run({
            containerName,
            imageName,
            onData: (data) => {
      ...
      if (error.errno === -13) {
        console.error(
          `permissions error detected -- be sure you've configured your docker image correctly (for linux users: follow the instructions at https://docs.docker.com/engine/install/linux-postinstall/`
        );
      }
      throw error;
    }
