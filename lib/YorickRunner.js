...
module.exports.YorickRunner = (runnerConfig) => {
  async function run() {
    ...
    try {
      ...
      console.log("BUILDING!", `docker build --tag=${imageTag} -f ${runnerConfig.dockerfile} .`)
      const buildResult = cp.execSync(`docker build --tag=${imageTag} -f ${runnerConfig.dockerfile} .`).toString()
      console.log("BUILT!")
